...
public class SpringcloudsecurecliApplication implements CommandLineRunner{
	...
	public void run(String... arg0) throws Exception {
		...
		resourceDetails.setAccessTokenUri("http://localhost:6000/services/oauth/token");

		// must be a valid scope or get an error; if empty, get all scopes
		// (default); better to ask for one
		...
		// diff user results in diff authorities/roles coming out; preauth on
		// roles fails for adam, works for barry
		...
		String s = template.getForObject("http://localhost:9011/services/tolldata", String.class);
